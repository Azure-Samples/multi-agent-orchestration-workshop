...
1. If you already have the `workshop` directory, change the name to something else like `workshop-02`.
...
1. In your terminal, run the following command to copy the start project to the workshop directory.
    ...
    mkdir -p $REPOSITORY_ROOT/workshop && \
        cp -a $REPOSITORY_ROOT/samples/02-concurrent-pattern/start/. $REPOSITORY_ROOT/workshop/
    ```

    ```powershell
    # PowerShell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/workshop -Force && `
        Copy-Item -Path $REPOSITORY_ROOT/samples/02-concurrent-pattern/start/* -Destination $REPOSITORY_ROOT/workshop -Recurse -Force
    ```
