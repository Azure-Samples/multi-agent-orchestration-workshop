...
1. If you already have the `workshop` directory, change the name to something else like `workshop-03`.
...
1. In your terminal, run the following command to copy the start project to the workshop directory.
    ...
    mkdir -p $REPOSITORY_ROOT/workshop && \
        cp -a $REPOSITORY_ROOT/samples/03-handoff-pattern/start/. $REPOSITORY_ROOT/workshop/
    ```

    ```powershell
    # PowerShell
    New-Item -Type Directory -Path $REPOSITORY_ROOT/workshop -Force && `
        Copy-Item -Path $REPOSITORY_ROOT/samples/03-handoff-pattern/start/* -Destination $REPOSITORY_ROOT/workshop -Recurse -Force
    ```
...
1. Open `src/MultiAgentWorkshop.Agent/Program.cs`, find the comment `// Create AzureOpenAIClient instance with EntraID authentication` and add the code right underneath it. This connects to the Microsoft Foundry project.
   ...
   - `endpoint.GetAzureOpenAIResponsesEndpoint()`: This converts the Microsoft Foundry project endpoint to the Microsoft Foundry OpenAI endpoint because the Microsoft Foundry SDK doesn't currently support the handoff pattern.
   - `new AzureOpenAIClient(url, credential)`: This connects to the Azure OpenAI instance using the endpoint and login details and converts it to an `IChatClient` instance.
